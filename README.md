...
## Трекінг помилок (Sentry)
У проєкті інтегровано платформу **Sentry** для збору аналітики та креш-репортів з пристроїв користувачів.
- Всі критичні помилки з `LogManager` автоматично дублюються у `SentrySDK.capture(message:)`.
- При падінні додатку (Crash), дамп пам'яті, стан пристрою (батарея, пам'ять, версія iOS) та хлібні крихти (breadcrumbs - останні дії користувача) автоматично відправляються на сервер при наступному запуску.
